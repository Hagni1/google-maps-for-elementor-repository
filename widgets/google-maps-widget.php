...
class Google_Maps_Widget extends Widget_Base
{
    ...
    protected function render()
    {
        ...
        echo '<div id="gme-map" style="' . esc_attr($style) . '" data-center-lat="' . esc_attr($center_lat) . '" data-center-lng="' . esc_attr($center_lng) . '" data-zoom="' . esc_attr($settings['zoom']) . '"></div>';
    }

    protected function _content_template()
    {
        ?>
        <# var center_lat, center_lng; if ( settings.center_type==='manual' && settings.center_lat && settings.center_lng ) {
            center_lat=settings.center_lat; center_lng=settings.center_lng; } else if ( settings.locations &&
            settings.locations.length> 0 ) {
            var total_lat = 0, total_lng = 0, count = 0;
            _.each(settings.locations, function(loc) {
            if ( loc.lat && loc.lng ) {
            total_lat += parseFloat(loc.lat);
            total_lng += parseFloat(loc.lng);
            count++;
            }
            });
            if ( count > 0 ) {
            ...
            #>
            <div id="gme-map" style="{{ style }}" data-center-lat="{{ center_lat }}" data-center-lng="{{ center_lng }}"
                data-zoom="{{ settings.zoom }}"></div>
            <?php
    }
}
